...
from drchrono.endpoints import DoctorEndpoint, PatientEndpoint, AppointmentProfileEndpoint, AppointmentEndpoint
import datetime
from .api import Api
...
class DoctorWelcome(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["appointment"] = api.appointments_list(start='2021-01-01', end='2021-06-01')[0]
...
def appointments_list(request):
    ...
    appointments = api.appointments_list(start=start, end=end, params = {'deleted_flag': False})

    # Updating schedule
    if(bool(form.data.get("update"))):
        api.appointments_update(id=form.data.get("id"),data = {
    ...
    if form.is_valid():
        ...
        logger.info("views: username: "+username)
        logger.info("views: password: "+password)
        ####
        # oauth_provider = UserSocialAuth.objects.get(provider="drchrono")
        # access_token = oauth_provider.extra_data["access_token"]
        # oauth_provider.disconnect(request)
    ...
    # # patients_dict
    # patients_dict = {}
    # patients_list = api.patients_list(params={'doctor':doctor['id']})
    # for p in patients_list:
    #     # patients_first_name[p['id']] = p['first_name']
    #     # patients_last_name[p['id']] = p['last_name']
    #     patients_dict[p['id']] = p
...
def appointments_create(request):
    ...
    return render(request, "appointments_create.html", {
        "form" : form,
        "title" : "My Form 123",
        "doctor" : doctor,
        "patients": patients,
